...
class RobotEmitterReceiver(ABC):
    ...
    initialize_comms methods are all abstract and need to be implemented, according to their docstrings.
    ...
    def __init__(
        ...
        Initializes the Webots Robot and sets up a basic timestep if None is supplied.

        Also initializes the emitter and the receiver used to communicate with the supervisor, using the
        initialize_comms() method which must be implemented by the user.

        The two methods handle_emitter() and handle_receiver() must also be implemented by the user.
    ...
    def handle_emitter(self):
        """
        This method should take data from the robot, eg. sensor data, parse it into a message and use the robot's
        emitter to send it to the supervisor. This message will be used as the observation of the robot.
        """
        pass
    ...
    def handle_receiver(self):
        """
        This method should take data through the receiver in the form of a message and parse into data usable by the
        robot.

        For example the message might include a motor speed, which should be parsed and applied to the robot's motor.
        """
        pass

    def run(self):
        """
        This method is required by Webots to update the robot in the simulation. It steps the robot and in each
        step it runs the two handler methods to use the emitter and receiver components.
